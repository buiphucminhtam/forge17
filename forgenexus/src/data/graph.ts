...
interface AdjEntry { uid: string; degree: number; neighbors: Set<string> }
...
export function detectCommunities(db: ForgeDB): Community[] {
  ...
  const allEdges = (db as any).db.prepare(
    `SELECT from_uid, to_uid, type FROM edges WHERE type IN (${placeholders})`
  ).all(...COMMUNITY_EDGE_TYPES) as any[];

  const adj = new Map<string, AdjEntry>();
  for (const e of allEdges) {
    for (const pair of [[e.from_uid, e.to_uid], [e.to_uid, e.from_uid]]) {
      if (!adj.has(pair[0])) {
  ...
  for (const [cid, members] of commMap) {
    const names = [...members]
      .map((u) => nodeNames.get(u) ?? '').filter(Boolean)
      .slice(0, 8);
...
export function traceProcesses(db: ForgeDB): Process[] {
  const entries = (db as any).db.prepare(`
  ...
  for (const entry of entries) {
    ...
    else if (fn.includes('event') || fn.includes('listener') || fn.includes('consumer')) type = 'event';
