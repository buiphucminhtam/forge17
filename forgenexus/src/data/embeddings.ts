...
export function detectProvider(): EmbeddingProvider {
  ...
  // Try Ollama
  const ollamaTest = (process.env.OLLAMA_HOST ?? 'http://localhost:11434').replace(/\/$/, '');
  // Don't await — just return ollama as likely available if host is set
...
export async function generateEmbeddings(
  ...
  const symbols = (db as any).db.prepare(
    "SELECT uid, name, type, file_path, signature FROM nodes WHERE embedding IS NULL LIMIT 50000"
  ).all() as any[];

  if (symbols.length === 0) {
    return { count: 0, model: opts.model ?? 'unknown', elapsedMs: Date.now() - start, provider: opts.provider };
  }
  ...
  let pendingEmbeddings: number[][] = [];

  console.error(`[ForgeNexus] Embeddings: using ${opts.provider} (${opts.model}) for ${symbols.length} symbols...`);
...
async function generateTransformersEmbeddings(
  ...
  if (!(globalThis as any).__forgeNexusExtractor) {
    console.error(`[ForgeNexus] Loading embedding model: ${model} (first run — downloading if needed)`);
  ...
  async function fetchOne(idx: number, text: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(`[ForgeNexus] Gemini batch error (${response.status}): ${err.substring(0, 100)}`);
        return;
      }
...
export async function findSimilar(
  ...
  const nodesWithEmbeddings = (db as any).db.prepare(
    "SELECT uid, name, file_path, embedding FROM nodes WHERE embedding IS NOT NULL"
  ).all() as any[];
...
function reciprocalRankFusion(
  ...
  const uidToScore = new Map<string, { name: string; filePath: string; rrfScore: number; sources: Set<string> }>();
...
/**
 * BM25 scoring — the classic IR ranking function.
 * Okapi BM25 with standard parameters: k1=1.5, b=0.75.
 */
function bm25Score(docTerms: string[], queryTerms: string[]): number {
  ...
  for (const qt of queryTerms) {
    if (termFreq.has(qt)) {
      score += termFreq.get(qt)!; // simplified BM25
    }
  }
  return score;
}
...
export async function hybridSearch(
  ...
  const processes = (db as any).db.prepare(
    "SELECT entry_point_uid, name FROM processes LIMIT 50"
  ).all() as any[];
