...
export class UnifiedGraph {
   ...
   */
  search(query: string, limit = 20): { repo: string; uid: string; name: string; filePath: string; type: string }[] {
    const results: { repo: string; uid: string; name: string; filePath: string; type: string; rank: number }[] = [];
    let globalRank = 0;
  ...
  getCrossRepoImports(): { fromRepo: string; fromUid: string; toRepo: string; toUid: string }[] {
    ...
    for (const [fromRepo, fromDb] of this.connections) {
      const importEdges = fromDb.prepare(
      ...
      for (const edge of importEdges) {
        ...
        for (const [toRepo, toDb] of this.connections) {
          ...
          const resolved = toDb.prepare(
            "SELECT uid FROM nodes WHERE file_path LIKE ? LIMIT 1"
   ...
   */
  getAllCommunities(): { repo: string; communities: { id: string; name: string; cohesion: number; count: number }[] }[] {
    const results: { repo: string; communities: { id: string; name: string; cohesion: number; count: number }[] }[] = [];

    for (const [repoName, db] of this.connections) {
      const rows = db.prepare('SELECT id, name, cohesion FROM communities ORDER BY cohesion DESC').all() as any[];
      results.push({
        repo: repoName,
