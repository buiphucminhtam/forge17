...
function runStatusFallback(repoPath: string, dbPath: string): void {
  try {
    ...
    try {
      indexedAt = execSync(
        `sqlite3 "${dbPath}" "SELECT value FROM meta WHERE key='indexed_at'"`
      , { encoding: "utf8" }).trim();
      ...
      if (isGit && lastCommit) {
        ...
        try {
          lastCommitTime = execSync(
            `git log -1 --format='%ci' ${lastCommit}`, { encoding: "utf8" }
          ).trim();
        } catch { /* ok */ }
      }
    } catch (e) {
    ...
    // Get counts
    let files = 0, nodes = 0, edges = 0, communities = 0, processes = 0;
    try {
      files = parseInt(execSync(
        `sqlite3 "${dbPath}" "SELECT COUNT(DISTINCT file_path) FROM nodes"`
      , { encoding: "utf8" }).trim()) || 0;
      nodes = parseInt(execSync(
        `sqlite3 "${dbPath}" "SELECT COUNT(*) FROM nodes"`
      , { encoding: "utf8" }).trim()) || 0;
      edges = parseInt(execSync(
        `sqlite3 "${dbPath}" "SELECT COUNT(*) FROM edges"`
      , { encoding: "utf8" }).trim()) || 0;
      communities = parseInt(execSync(
        `sqlite3 "${dbPath}" "SELECT COUNT(*) FROM communities"`
      , { encoding: "utf8" }).trim()) || 0;
      processes = parseInt(execSync(
        `sqlite3 "${dbPath}" "SELECT COUNT(*) FROM processes"`
      , { encoding: "utf8" }).trim()) || 0;
    } catch { /* ok */ }
    ...
    console.log(`  Commit:   ${lastCommit || "none"} ${lastCommitTime ? `(${lastCommitTime})` : ""}`);
    if (isGit) {
    ...
    // Edge type breakdown
    let edgeTypes: Record<string, number> = {};
    try {
      const raw = execSync(
