...
async function loadLanguage(lang: string): Promise<any> {
  switch (lang) {
    ...
    case 'go': {
      try { return (_require('tree-sitter-go') as any).default ?? null; } catch { return null; }
    }
    case 'rust': {
      try { return (_require('tree-sitter-rust') as any).default ?? null; } catch { return null; }
    }
    case 'java': {
      try { return (_require('tree-sitter-java') as any).default ?? null; } catch { return null; }
    }
    case 'csharp': {
      try { return (_require('tree-sitter-c-sharp') as any).default ?? null; } catch { return null; }
    }
    case 'cpp': {
...
function getIdentifier(node: any, content: string): string | null {
  for (let i = 0; i < node.namedChildCount; i++) {
    const child = node.namedChild(i);
    if (child.type === 'identifier') return getNodeText(child, content);
  }
  return null;
}

function getFirstChildOfType(node: any, type: string): any {
  return node.children?.find((c: any) => c.type === type) ?? null;
}
...
function getNodeMap(lang: string): Record<string, NodeMapping> {
  switch (lang) {
    case 'typescript': case 'tsx': case 'javascript': return NODE_MAP_TS;
    case 'python': return NODE_MAP_PY;
    case 'go': return NODE_MAP_GO;
    case 'rust': return NODE_MAP_RUST;
    case 'java': return NODE_MAP_JAVA;
    case 'csharp': return NODE_MAP_CS;
    case 'cpp': return NODE_MAP_CPP;
    case 'c': return NODE_MAP_C;
    case 'kotlin': return NODE_MAP_KOTLIN;
    case 'php': return NODE_MAP_PHP;
    case 'ruby': return NODE_MAP_RUBY;
    case 'swift': return NODE_MAP_SWIFT;
    case 'dart': return NODE_MAP_DART;
    default: return NODE_MAP_TS;
  }
}
...
export class ParserEngine {
  ...
  async parseFile(filePath: string, content: string, language?: string): Promise<{ nodes: CodeNode[]; edges: CodeEdge[] }> {
  ...
  private getDeclarationName(node: any, content: string, mapping: NodeMapping): string {
    ...
    if (node.type === 'function_declaration' || node.type === 'function_definition'
      || node.type === 'function_item' || node.type === 'method_declaration'
    ...
    if (node.type === 'class_declaration' || node.type === 'class_definition'
      ...
      const nameNode = getChild(node, 'type_identifier')
    ...
    if (node.type === 'property_signature' || node.type === 'property_definition'
      || node.type === 'public_field_definition' || node.type === 'property_declaration'
      || node.type === 'field_definition' || node.type === 'field_declaration'
      || node.type === 'constant_declaration' || node.type === 'instance_variable_declaration') {
      const nameNode = getChild(node, 'property_identifier')
        ?? getChild(node, 'string')
        ?? getChild(node, 'identifier')
        ?? getChild(node, 'variable_name');
      if (nameNode) return getNodeText(nameNode, content).replace(/['"`]/g, '');
    }
    if (node.type === 'parameter' || node.type === 'formal_parameter'
    ...
    if (node.type === 'method_declaration') {
      const nameNode = getChild(node, 'name') ?? getChild(node, 'property_identifier') ?? getChild(node, 'identifier');
      if (nameNode) return getNodeText(nameNode, content);
    }
  ...
  private extractImportEdges(node: any, parentUid: string, content: string, edges: CodeEdge[]): void {
    ...
    if (node.type === 'import_statement') {
      ...
      const sourceNode = getChild(node, 'string') ?? node.children?.find((c: any) => c.type === 'string' || c.type === 'external_module_reference');
    ...
    if (node.type === 'import_declaration') {
      ...
      const sourceNode = getChild(importSpec, 'string') ?? importSpec.children?.find((c: any) => c.type === 'string');
      if (sourceNode) {
        const source = getNodeText(sourceNode, content).replace(/['"`]/g, '');
    ...
    if (node.type === 'import_declaration') {
      const text = getNodeText(node, content).replace(/^import\s+/, '').replace(/;$/, '').trim();
      if (text) {
        const parts = text.split('.');
    ...
    // Swift: import Module
    if (node.type === 'import_declaration') {
      const moduleNode = getChild(node, 'identifier') ?? node.children?.find((c: any) => c.type === 'identifier');
      if (moduleNode) {
    ...
    if (node.type === 'import_directive') {
      const uriNode = getChild(node, 'uri') ?? node.children?.find((c: any) => c.type === 'uri' || c.type === 'string_literal');
      const source = uriNode ? getNodeText(uriNode, content).replace(/['"`]/g, '') : '';
      const prefix = getChild(node, 'hide') ?? node.children?.find((c: any) => c.type === 'identifier' && c.text !== 'hide' && c.text !== 'show');
      if (source) {
        const toUid = `IMPORT:${source}:module`;
        edges.push({
  ...
  private extractHeritageEdges(node: any, parentUid: string, content: string, filePath: string, edges: CodeEdge[]): void {
        ...
        if (child.type === 'class_heritage') {
          for (const heritageChild of child.namedChildren) {
            if (heritageChild.type === 'extends_clause') {
              const superClass = getChild(heritageChild, 'type_identifier') ?? getChild(heritageChild, 'identifier');
              if (superClass) {
    ...
    if (node.type === 'struct_declaration' || node.type === 'type_declaration') {
      ...
      const structType = typeSpec ? getChild(typeSpec, 'struct_type') : getChild(node, 'struct_type');
      if (structType) {
    ...
    if (node.type === 'class_declaration') {
      const classBody = getChild(node, 'base_list') ?? node.namedChildren.find((c: any) => c.type === 'base_list');
      if (classBody) {
    ...
    if (node.type === 'class_declaration' || node.type === 'struct_declaration') {
      for (const child of node.namedChildren) {
        if (child.type === 'inheritance_specifier') {
          const typeNodes = getNamedChildren(child, 'type_identifier') ?? getNamedChildren(child, 'identifier');
  ...
  private extractMemberEdges(node: any, parentUid: string, content: string, filePath: string, edges: CodeEdge[]): void {
    const isContainer = node.type === 'class_declaration' || node.type === 'interface_declaration'
      || node.type === 'class_definition' || node.type === 'struct_item'
      || node.type === 'impl_item' || node.type === 'class_body'
      // Java
    ...
    if (node.type === 'struct_declaration' || node.type === 'struct_item') nodeTypeForMembers = 'Class';

    const containerUid = `${filePath}:${nodeTypeForMembers}:${containerName}:0`;

    // Find body — language-specific
    let body: any = null;
    const bodyNames = ['class_body', 'declaration_list', 'block', 'declaration',
      'body', 'member_block', 'members', 'class_body2'];
    ...
    if (body) {
      for (const member of body.namedChildren) {
        if (member.type === 'method_definition' || member.type === 'method_signature'
          || member.type === 'function_declaration' || member.type === 'function_item'
          || member.type === 'function_definition' || member.type === 'method_declaration'
          || member.type === 'function_declaration' && member.language?.name?.includes('kotlin')
          || member.type === 'method' || member.type === 'singleton_method'
          || member.type === 'property_declaration' || member.type === 'function_definition'
          || member.type === 'field_definition' || member.type === 'property_definition') {
        ...
        if (member.type === 'property_signature' || member.type === 'public_field_definition'
          || member.type === 'property_definition' || member.type === 'field_declaration'
          || member.type === 'property_declaration' || member.type === 'constant_declaration'
          || member.type === 'assignment' || member.type === 'field_definition'
  ...
  private getMemberName(node: any, content: string): string | null {
    ...
    if (node.type === 'function_declaration' || node.type === 'function_definition' || node.type === 'function_item'
      || node.type === 'method_declaration' || node.type === 'method') {
      const id = getChild(node, 'identifier') ?? getChild(node, 'property_identifier') ?? getChild(node, 'string');
      if (id) return getNodeText(id, content);
      const memberExpr = getChild(node, 'member_expression');
      if (memberExpr) {
        const prop = getChild(memberExpr, 'property_identifier');
    ...
    if (node.type === 'property_signature' || node.type === 'public_field_definition'
      || node.type === 'property_definition' || node.type === 'field_declaration'
      || node.type === 'property_declaration' || node.type === 'constant_declaration'
      || node.type === 'field_definition' || node.type === 'assignment'
  ...
  private extractAccessEdges(node: any, parentUid: string, content: string, edges: CodeEdge[]): void {
    if (node.type === 'member_expression') {
      ...
      if (prop) {
        ...
        const parent = node.parent;
        const isWrite = parent?.type === 'assignment_expression'
          && parent.children?.[0]?.type === 'member_expression'
          && parent.children?.[0]?.text === prop.text;
        edges.push({
      ...
      if (fn && /^(app|router|express|route|use)\./.test(fn)) {
        if (/^\s*(get|post|put|patch|delete|head|options|all)\s*\(/.test(text)) {
          ...
          if (args.length > 0) {
            ...
            for (const arg of argList.namedChildren) {
              if (arg.type === 'identifier' || arg.type === 'arrow_function'
                || arg.type === 'function_expression') {
                const handlerName = arg.type === 'identifier'
  ...
  private extractMemberOfEdges(node: any, parentUid: string, content: string, filePath: string, edges: CodeEdge[]): void {
    const memberTypes = new Set([
      'method_definition', 'function_definition', 'arrow_function',
      'property_declaration', 'variable_declarator', 'assignment_expression',
      'method_declaration', 'class_static_block_declaration',
    ...
    const containerTypes = new Set([
      'class_declaration', 'class_body', 'interface_declaration',
      'class_definition', 'struct_item', 'impl_item', 'enum_declaration',
  ...
  private extractProcessEdges(node: any, parentUid: string, content: string, filePath: string, edges: CodeEdge[]): void {
    ...
    const entryTypes = new Set([
      'call_expression', 'function_declaration', 'method_definition',
