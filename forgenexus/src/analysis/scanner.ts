...
const DEFAULT_SKIP = [
  ...
  '**/coverage/**', '**/*.min.js', '**/*.map', '**/*.d.ts',
  '**/__pycache__/**', '**/vendor/**', '**/android/**', '**/ios/**',
];
...
export class FileScanner {
  ...
  async scan(): Promise<ScannedFile[]> {
    ...
    const patterns = exts.flatMap((ext) => [`**/*${ext}`, ...this.config.skipPatterns.map((s) => `!${s}`)]);
  ...
  detectLanguage(files: ScannedFile[]): string {
    ...
    let max = 'unknown', maxCount = 0;
    for (const [l, c] of counts) { if (c > maxCount) { maxCount = c; max = l; } }
    return max;
  }
}
