...
import os from 'os';
// ─── Helpers ─────────────────────────────────────────────────────────
async function withWorkspace(rootVar, fn) {
...
function writeState(wsDir, state) {
...
describe('getForgewrightRoot', () => {
    it('should return a valid absolute path', async () => {
        const { getForgewrightRoot } = await import('./pipeline-manager.js');
        const root = getForgewrightRoot();
        expect(path.isAbsolute(root)).toBe(true);
        expect(fs.existsSync(root)).toBe(true);
    });
    it('should return the forgewright root (has mcp/ subdirectory)', async () => {
        const { getForgewrightRoot } = await import('./pipeline-manager.js');
        const root = getForgewrightRoot();
        expect(fs.existsSync(path.join(root, 'mcp'))).toBe(true);
    });
});
describe('getState / saveState', () => {
    ...
    it('should use atomic write — no temp files left behind', async () => {
        await withWorkspace('FORGEWRIGHT_ROOT', async (dir) => {
            ...
            saveState({ currentPhase: 0, currentMode: null, history: [], status: 'IDLE' });
            const fwDir = path.join(dir, '.forgewright');
...
describe('resetWorkspaceRoot', () => {
    ...
    it('should allow workspace root to be reset and re-set', async () => {
        ...
        try {
            ...
            vi.resetModules();
            const { setWorkspaceRoot, resetWorkspaceRoot, getWorkspaceRoot } = await import('./pipeline-manager.js');
            setWorkspaceRoot();
